...
class TemplateGenerator {
   ...
   */
  _create( options = {} ) {
    //   { type: 'single', name: 'vue-123/index', isDir: undefined }
    //   { type: 'single', name: 'vue-123', isDir: undefined }
  ...
  _getSingleTpl( type , extension = 'js') {
    if(type === 'single') {
      ...
      return `${this.TEMPLATES_DIR}/spec/temp.spec.js`;
    }
  ...
  _createFilePath( name, type, fileType ) {
    if(type === 'single') {
      return path.join(process.cwd(), `${name}.spec.js`);
    }
